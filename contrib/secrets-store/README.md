...
Why is this integration important? K8s secrets, while convenient, are sometimes deemed insufficient for high-security environments.

The integration of ODM that runs on Kubernetes with an external secret store via the Secrets Store CSI Driver offers a more secure and efficient way to handle sensitive data.
...
We will use [Hashicorp Vault as a secrets store](https://developer.hashicorp.com/vault/tutorials/kubernetes/kubernetes-secret-store-driver) and OpenShift Container Platform (OCP) as a Kubernetes cluster for this article.
...
- **Secret Manager Server**: It serves as the central repository for all secrets data, securely managing sensitive information.

- **Secrets Data**: This represents the actual sensitive information that needs to be securely managed and injected into the ODM Pods.

- **Secrets Provider Class**: Definition of the data that should be injected into the ODM Pods.
...
- **ODM Pods**:
  - Detailed within the Kubernetes rectangle, showcasing the components that make up the ODM Pods:
    ...
    - **ODM Containers**:
      ...
      - They use the secrets data stored in the volume for secure operations and configuration.
...
HashiCorp Vault must be up and running. An [on-premises installation description](README-External_Vault.md) is provided (with hints about the Secrets Store CSI driver and the HashiCorp Vault provider installation), but of course you can use your own instance.
...
Most of the following commands require you to be already connected to your secrets store:
...
Create the `odm-policy` policy to enable the read capability for secrets at path `<secretspath>/data/`:
...
Create an authentication role which assigns the policy created above to some `odm-sa` service account in the namespace `odm` on your OCP cluster:
...
A few mandatory items must be created so that ODM can be deployed.
...
We also need to create a Basic Registry configuration to be used as authSecretRef (refer to the accompanying files group-security-configurations.xml and webSecurity.xml). This will allow a user named "mat" to connect to ODM components. First, upload its contents to HashiCorp Vault:
...
2. Run Helm deployment with the CSI driver (more about supported secrets references and values format at the bottom of this page):
...
Each reference can be either a Kubernetes Secret or a CSI SecretProviderClass. Historically there was only one way to specify a reference:
...
It means: \<name\> is the name of a Kubernetes secret.

From this release (ODM on K8s 9.6.0) an additional format is also supported:
...
Now \<name\> is the name either of a Kubernetes secret or of a CSI SecretProviderClass, depending on type: value ('secret' being the default type).

The "historical" way is kept for compatibility during upgrades.
