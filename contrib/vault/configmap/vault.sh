...
configure_decisions() {
  ...
  "odm.oidc-registration") configure_decisions_registration_job;;
...
configure_decisions_odm_pod() {
  ...
  # CONTAINERENVFILE=container.env

  echo "Retrieved Token from custom claim"

  if [ -f "/ibm/icp4ba/pv/token" ]; then
      echo "Found PV and token file"
      cat /ibm/icp4ba/pv/token
  else
      echo "No PV found."
      # touch /ibm/icp4ba/pv/token
  fi

  echo "Extract DB information from Vault and put information in /script/init/container.env file"

  echo "LS STORE -----"
  ls -1 /mnt/secrets-store/*
  echo "ENDLS"
  ...
  # TODO To be docuemted
#  echo OPENID_CLIENT_ID="odm-oidc-client-id-secret" >> $CONTAINERENVFILE
#  echo OPENID_CLIENT_SECRET="odm-oidc-client-id-secret-value" >> $CONTAINERENVFILE
